...
namespace TwentyFiveSlicer.Runtime
{
    ...
    public class TwentyFiveSliceImage : Image
    {
        ...
        private void PlaceFixedSlices(SliceRect[,] slices, Rect targetRect, float[] xBorders, float[] yBorders)
        {
            ...
            PlaceFixedSlice(slices, 4, 0, new Vector2(targetRect.xMax - originalWidths[4], targetRect.yMin),
                originalWidths[4], originalHeights[0]); // 5
            PlaceFixedSlice(slices, 0, 4, new Vector2(targetRect.xMin, targetRect.yMax - originalHeights[4]),
                originalWidths[0], originalHeights[4]); // 21
            PlaceFixedSlice(slices, 4, 4,
                new Vector2(targetRect.xMax - originalWidths[4], targetRect.yMax - originalHeights[4]),
                originalWidths[4], originalHeights[4]); // 25

            // 가장자리 중앙 (3, 11, 15, 23)
            float centerX = targetRect.center.x;
            float centerY = targetRect.center.y;

            PlaceFixedSlice(slices, 2, 0, new Vector2(centerX - originalWidths[2] * 0.5f, targetRect.yMin),
                originalWidths[2], originalHeights[0]); // 3
            PlaceFixedSlice(slices, 0, 2, new Vector2(targetRect.xMin, centerY - originalHeights[2] * 0.5f),
                originalWidths[0], originalHeights[2]); // 11
            PlaceFixedSlice(slices, 4, 2,
                new Vector2(targetRect.xMax - originalWidths[4], centerY - originalHeights[2] * 0.5f),
                originalWidths[4], originalHeights[2]); // 15
            PlaceFixedSlice(slices, 2, 4,
                new Vector2(centerX - originalWidths[2] * 0.5f, targetRect.yMax - originalHeights[4]),
                originalWidths[2], originalHeights[4]); // 23

            // 중앙 (13)
            PlaceFixedSlice(slices, 2, 2,
                new Vector2(centerX - originalWidths[2] * 0.5f, centerY - originalHeights[2] * 0.5f), originalWidths[2],
                originalHeights[2]); // 13
        }
        ...
        private void PlaceFullyStretchableSlices(SliceRect[,] slices, Rect targetRect, float[] xBorders,
            float[] yBorders)
        {
